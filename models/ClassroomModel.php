...
class ClassroomModel extends CModel {
    ...
    public function getroomdetailbydomain($domain) {
        ...
        $sql = "select cr.crid,cr.uid,cr.crname,cr.domain,cr.template,cr.upid,cr.isschool,cr.dateline,cr.summary,cr.crlabel,cr.cface,cr.banner,cr.crqq,cr.craddress,cr.crphone,cr.cremail,cr.modulepower,cr.stumodulepower,cr.lng,cr.lat,cr.crprice,cr.tplsetting,cr.grade,cr.ispublic,cr.coursenum from ebh_classrooms cr where cr.domain=$domain";
    ...
    function getroomlistbytid($tid) {
        $sql = 'select c.crid,c.crname,c.domain,c.upid,c.cface,c.summary,c.coursenum,c.teanum,c.stunum from ebh_roomteachers rt ' .
    ...
    function getroomlistbyuid($uid) {
        ...
        $sql = 'select c.crid,c.crname,c.domain from ebh_roomusers rc ' .
...
/*
	...
	*/
	public function getclassroomlist($param){
		$sql = 'select c.crid,c.crname,c.begindate,c.crprice,c.upid,c.enddate,c.dateline,c.maxnum,c.domain,c.status,c.summary,u.realname,u.nickname,u.username,c.displayorder,uu.username as agentname from ebh_classrooms c join ebh_users u on u.uid = c.uid left join ebh_users uu on uu.uid=c.agentid';
		if(isset($param['q']))
...
//log_message($sql);
	...
	public function getdetailclassroom($crid) {
		$sql = "select cr.crid,cr.uid,cr.crname,cr.domain,cr.template,cr.isschool,cr.summary,cr.crlabel,cr.cface,cr.crqq,cr.craddress,cr.crphone,cr.cremail,cr.modulepower,cr.stumodulepower,cr.bankcard,"
				. "cr.dateline,cr.banner,cr.displayorder,cr.viewnum,cr.score,cr.onlinecount,cr.lng,cr.lat,cr.weibosina,cr.stunum,cr.teanum,cr.coursenum,cr.message,cr.good,cr.bad,cr.useful,cr.districts,cr.examcount  from ebh_classrooms cr where cr.crid=$crid";
		return $this->db->query($sql)->row_array();
	}
	/*
	多个教室详情
	...
	*/
	public function getRoomByCrid($crid) {
		$domain = '';
		$sql = 'select cr.crid,cr.uid,cr.crname,cr.domain,cr.template,cr.upid,cr.isschool,cr.dateline,cr.summary,cr.crlabel,cr.cface,cr.banner,cr.crqq,cr.craddress,cr.crphone,cr.cremail,cr.modulepower,cr.stumodulepower,cr.lng,cr.lat,cr.crprice,cr.tplsetting,cr.grade,cr.ispublic,cr.coursenum from ebh_classrooms cr where crid='.$crid; 
		return $this->db->query($sql)->row_array();
	}
	/**
	*添加教室的评分数
	...
	*/
	public function getUsernameByRealname($param){
		...
		$sql.= " where realname='$realname' and sex=$sex and crname='$crname'";
		// echo $sql;
		return $this->db->query($sql)->row_array();
	}
	...
	*/
	public function getUproom($domain){
		$sql = 'select cr1.domain,cr1.crname from ebh_classrooms cr1 join ebh_classrooms cr2 on cr1.crid=cr2.upid where cr2.domain=\''.$domain.'\'';
		// echo $sql;
		return $this->db->query($sql)->row_array();
