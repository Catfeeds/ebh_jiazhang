...
class ClassesModel extends CModel{
	...
	*/
	public function addclass($param){
		...
		$this->db->insert('ebh_classes',$setarr);
	}
	...
	*@param int $uid 用户编号
	*/
	public function getClassNocrid($uid) {
		$sql = "SELECT cs.classid,c.classname,c.grade,c.district from  ebh_classstudents cs ".
				"JOIN ebh_classes c on (c.classid = cs.classid) ".
				"WHERE cs.uid = $uid";
		return $this->db->query($sql)->row_array();
	...
	*/
	public function deleteMultiStudentFromClass($param){
		...
		$this->db->delete('ebh_classstudents','classid='.$param['classid']);
		if(!empty($param['uids'])){
			$sql = 'delete from ebh_roomusers where crid='.$param['crid'].' and uid in ('.$param['uids'].')';
			$this->db->query($sql);
		}
		$this->db->update('ebh_classes',array(),array('classid'=>$param['classid']),array('stunum'=>'stunum-'.$param['stunum']));
