...
<div class="workol">
	<div class="work_mes" style="width:998px; position:relative;">
		<ul style="float:left; display:inline;">
			...
			<li class="workcurrent"><a href="<?= geturl('myask/settled') ?>"><span>已解决</span></a></li>
										...
										<td style="border-top:none;">
											<?php $name=empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>
												<div style="float:left;margin-right:15px;"><a href="<?= geturl('myask/'.$avalue['qid']) ?>" target="_blank"><img title="<?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>" src="<?=$face?>" /></a></div>
													...
													<span class="huirenw" style="width:180px;float:left;"><a href="settled.html?aq=<?= $name?>"><?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?></a></span>
													<span class="ketek"><a href="settled.html?fid=<?= $avalue['folderid']?>"><?= $avalue['foldername'] ?></a></span>
													
													</div>
													<?php if(!empty($avalue['cwid'])){?>
													<div style="float:left;width:600px;margin-top:5px;margin-left:150px;">
													<span class="cwsp" style="margin-left:294px"><a href="/myask/settled.html?cwid=<?=$avalue['cwid']?>"><?=shortstr($avalue['cwname'],40)?></a></span>
													</div>
													<?php }?>
												</div>
...
$(function() {
	...
	$("#ser").click(function(){
       ...
       if(title == searchtext) 
           title = "";
       var url = '<?= geturl('myask/settled') ?>' + '?q='+title;
	   <?php if(!empty($folder)){
