...
<div class="workol">
	<div class="work_mes" style="width:998px; position:relative;">
		<ul style="float:left; display:inline;">
			...
			<li class="workcurrent"><a href="<?= geturl('myask/recommend') ?>"><span>推荐</span></a></li>
									...
									<tr>
										<td style="border-top:none;">
											<?php $name=empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>
												<div style="float:left;margin-right:15px;"><a href="<?= geturl('myask/'.$avalue['qid']) ?>" target="_blank"><img title="<?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?>的个人空间" src="<?=$face?>" /></a></div>
													...
													<span class="huirenw" style="width:180px;float:left;"><a href="recommend.html?aq=<?= $name?>"><?= empty($avalue['realname'])?$avalue['username']:$avalue['realname'] ?></a></span>
													<span class="ketek"><a href="recommend.html?fid=<?= $avalue['folderid']?>"><?= $avalue['foldername'] ?></a></span>
													
													</div>
													<?php if(!empty($avalue['cwid'])){?>
													<div style="float:left;width:600px;margin-top:5px;margin-left:150px;">
													<span class="cwsp" style="margin-left:294px"><a href="/myask/recommend.html?cwid=<?=$avalue['cwid']?>"><?=shortstr($avalue['cwname'],40)?></a></span>
													</div>
													<?php }?>
												</div>
...
$(function() {
	...
	$("#ser").click(function(){
		...
		if(title == searchtext) 
           title = "";
		var url = '<?= geturl('myask/recommend') ?>' + '?q='+title;
		<?php if(!empty($folder)){
