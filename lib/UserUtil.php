...
class UserUtil{
     ...
     * @User:tzq
     * @Date:2017/12/12
     * @param string $field 要单独获取的字段
     * @return
     */
	public function getUserInfo($field=''){
       if(empty(self::$userInfo)){
           self::$userInfo = Ebh::app()->user->getloginuser();
       }
       if(empty($field))
           return self::$userInfo;
       return isset(self::$userInfo[$field])?self::$userInfo[$field]:FALSE;
	}
     ...
     * @return int
     */
	public function getRoomInfo($field = ''){
      if(empty(self::$roomInfo)){
          self::$roomInfo = Ebh::app()->room->getcurroom();
      }
      if(empty($field))
          return self::$roomInfo;
      if($field == 'type'){
          $property                 = isset(self::$roomInfo['property']) ? self::$roomInfo['property'] : 0;
          $isschool                 = isset(self::$roomInfo['isschool']) ? self::$roomInfo['isschool'] : 0;
          ...
          $is_zjdlr                 = (self::$roomInfo['crid'] == $other_config['zjdlr']) || (in_array(self::$roomInfo['crid'], $other_config['newzjdlr']));
          $is_newzjdlr              = in_array(self::$roomInfo['crid'], $other_config['newzjdlr']);
          if ($property == 3 && $isschool == 7) {
              $type = 1;//企业版
          } elseif ($is_newzjdlr || $is_zjdlr) {
              $type = 2;//国土版
          } elseif($isschool == 3 && $property ) {
              $type = 4;//租赁版
          }else{
              $type = 3;//其他版本 教育版
          }
      ...
      return isset(self::$roomInfo[$field]) ? self::$roomInfo[$field] : FALSE ;
	}
}