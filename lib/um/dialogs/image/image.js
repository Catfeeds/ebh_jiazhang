(function () {
    ...
    UM.registerWidget('image', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            ...
            "</div>" +
            "</div>",
        tpl2: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            "<div id=\"edui-image-Jwrapper\" class=\"edui-image-wrapper\">" +
            "<ul class=\"edui-tab-nav\">" +
            "<li class=\"edui-tab-item active\"><a href=\"#edui-image-Jlocal\" class=\"edui-tab-text\"><%=lang_tab_local%></a></li>" +
            "<li  class=\"edui-tab-item\"><a href=\"#edui-image-JimgSearch\" class=\"edui-tab-text\"><%=lang_tab_imgSearch%></a></li>" +
        ...
        initContent: function (editor, $dialog) {
            ...
            if (lang && !window.uidialog) {
                var html = $.parseTmpl(this.tpl, opt);
                this.root().html(html);
            }else{
                var html = $.parseTmpl(this.tpl2, opt);
                currentDialog = $dialog.edui();
                this.root().html(html);
                $("#edui-dialog-image .edui-modal-header").remove();
                $("#edui-dialog-image").dialog({
                    title:'图片上传',
                    width:715,
                    height:560,
                    modal:false,
                    close:function(){
                        $("div.edui-btn[data-hide=modal]").trigger('click');
                    }
                });
            }
            ...
            'ok': {
                exec: function (editor, $w) {
                    ...
                    if (index != -1) {
                        ...
                        if(window.uidialog){
                            $("#edui-dialog-image").dialog('close');
                        }
                    }
                }
            },
